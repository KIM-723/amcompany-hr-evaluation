      ...
      <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        ...
        <label className={label}>고용형태 *
          <select name="employment_type" defaultValue={value.employment_type ?? 'regular'} className={input}>
            <option value="regular">정규직</option><option value="contract">계약직</option><option value="part_time">파트타임</option><option value="executive">임원</option><option value="other">기타</option>
          </select>
        </label>
        <label className={label}>재직상태 *
          <select name="employment_status" defaultValue={value.employment_status ?? 'active'} className={input}>
            <option value="active">재직</option><option value="leave">휴직</option><option value="resigned">퇴사</option>
          </select>
        </label>
        <label className={label}>부서
          <select name="department_id" defaultValue={value.department_id ?? ''} className={input}>
            <option value="">미지정</option>{departments.map((x)=><option key={x.id} value={x.id}>{x.name}</option>)}
          </select>
        </label>
        <label className={label}>직급
          <select name="job_level_id" defaultValue={value.job_level_id ?? ''} className={input}>
            <option value="">미지정</option>{jobLevels.map((x)=><option key={x.id} value={x.id}>{x.name}</option>)}
          </select>
        </label>
        <label className={label}>직책
          <select name="position_id" defaultValue={value.position_id ?? ''} className={input}>
            <option value="">미지정</option>{positions.map((x)=><option key={x.id} value={x.id}>{x.name}</option>)}
          </select>
        </label>
        <label className={label}>리더
          <select name="leader_id" defaultValue={value.leader_id ?? ''} className={input}>
            <option value="">미지정</option>{leaders.map((x)=><option key={x.id} value={x.id}>{x.employee_no} · {x.name}</option>)}
          </select>
        </label>
