...
export async function startFirstEvaluation(assignmentId: string) {
  ...
  if (!evaluatorId) redirectMessage('/evaluations/first', 'error', '1차 평가자를 확인할 수 없습니다.');
  ...
  if (error) redirectMessage('/evaluations/first', 'error', error.message);

  await supabase.from('evaluation_assignments').update({ status: 'first_in_progress', current_stage: 'first' }).eq('id', assignmentId);
...
export async function saveFirstEvaluation(assignmentId: string, formData: FormData) {
  ...
  const { data: questions } = await supabase
    ...
    .in('id', formData.getAll('question_ids').map(String));
  ...
  for (const q of questions ?? []) {
    ...
    if (!score || score < 1 || score > 5) {
      redirectMessage(`/evaluations/first/${assignmentId}`, 'error', `${q.title}: 1~5점 점수를 입력해주세요.`);
    }
    if ((score === 1 || score === 5) && !evidenceNote) {
      redirectMessage(`/evaluations/first/${assignmentId}`, 'error', `${q.title}: 1점/5점은 근거가 필수입니다.`);
    }

    const { data: response, error } = await supabase.from('evaluation_responses').upsert(
      {
    ...
    if (error) redirectMessage(`/evaluations/first/${assignmentId}`, 'error', error.message);

    const observationId = optionalText(formData.get(`observation_${q.id}`));
    if (observationId) {
      const { data: ev } = await supabase.from('evaluations').select('evaluator_id').eq('id', evaluationId).single();
      await supabase.from('evaluation_evidence_links').upsert(
        {
          response_id: response.id,
          observation_log_id: observationId,
          linked_by: ev?.evaluator_id,
        },
  ...
  const scores = (questions ?? []).map(q => Number(formData.get(`score_${q.id}`))).filter(Number.isFinite);
  const totalScore = scores.length ? scores.reduce((a,b)=>a+b,0)/scores.length : null;
  const status = intent === 'submit' ? 'submitted' : 'draft';

  const { error: evaluationError } = await supabase.from('evaluations').update({
    status,
  ...
  if (evaluationError) redirectMessage(`/evaluations/first/${assignmentId}`, 'error', evaluationError.message);

  if (intent === 'submit') {
    await supabase.from('evaluation_assignments').update({ status: 'first_submitted', current_stage: 'second' }).eq('id', assignmentId);
  }

  revalidatePath(`/evaluations/first/${assignmentId}`);
  redirectMessage(`/evaluations/first/${assignmentId}`, 'success', intent === 'submit' ? '1차 평가를 제출했습니다.' : '1차 평가를 저장했습니다.');
}
