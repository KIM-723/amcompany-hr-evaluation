...
export function rolesCanAccessRoute(roles: readonly Role[], pathname: string): boolean {
  ...
  if (pathname.startsWith('/api/export/')) {
    return roles.some((role) => ['leader','hr_admin','super_admin'].includes(role));
  }
...
export function getNavigationForRole(role: Role) { return getNavigationForRoles([role]); }
export function getNavigationForRoles(roles: readonly Role[]) {
  return NAVIGATION_ITEMS.filter((item) => item.roles.some((role) => roles.includes(role))).map((item) => ({
    ...item,
    children: item.children?.filter((child) => child.roles.some((role) => roles.includes(role))),
  }));
}
